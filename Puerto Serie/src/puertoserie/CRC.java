...
public class CRC implements Checksum {
    ...
    public byte[] generarCRC(byte[] trama) {//originalmente recibe el arreglo como INT
        CRC crc = new CRC();
        byte[] data = trama;
        for (int d : data) {
            crc.update(d);
        ...
        byte[] crcGenerado = new byte[2];
        crcGenerado[0] = (byte) ((crc.getValue() & 0x000000ff));
        crcGenerado[1] = (byte) ((crc.getValue() & 0x0000ff00) >>> 8);
